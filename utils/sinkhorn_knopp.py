...
class SinkhornKnopp_im(torch.nn.Module):
    ...
    def forward(self, features, head, split=0, queue=None, return_logit=False):
        if queue is None or queue.shape[0] == 0:
            queue = None
        if queue is not None:
            features = torch.vstack((features, queue))
        
        features = torch.nn.functional.normalize(features, dim=1, p=2) #feature -> Z
        head = torch.nn.functional.normalize(head, dim=1, p=2) #head = prototypes -> C
