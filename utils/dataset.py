...
def get_dataset(name, type='disjoint'):
    if name == "SemanticKITTI":
        if type == 'labeled':
            ...
            return SemanticKITTIRestrictedDualLevelDataset
        else:
...
class SemanticKITTIRestrictedDualLevelDataset(torch.utils.data.Dataset):
    def __init__(
        self,
        config_file="config/dataset.yaml",
        data_idx_file='selected_idx.npy',
...
class nuScenesRestrictedFineTuningDataset(torch.utils.data.Dataset):
    ...
    def ds_sample(self, xyz, ref, labels, instances):        
        ...
        for instance_idx in inst_basic_idx:
            ...
            labels[instances == instance_idx] =  (16 - len(self.unknown_labels))
            # if dataset_nusc:
            # labels[instances == instance_idx] = 17
...
class SemanticPOSSRestrictedDataset(torch.utils.data.Dataset):
    def __init__(
        ...
        if split == "train" and augment:
            ...
            self.voxelizer = Voxelizer(
                ...
                translation_augmentation_ratio_bound=self.translation_augmentation_ratio_bound,
                ignore_label=-1,
            ) #rotation, translation augmented 된 voxel 생성

        for sequence in self.config["split_sequence"][split]: #split = train
            for idx, type in enumerate(self.files.keys()):
                files_path = os.path.join(
                    self.config["dataset_path"],
                    "sequences",
                ...
                if len(data) == 0:
                    raise RuntimeError("Missing data for " + type)
                self.files[type].extend(data) #self.files에 point cloud 불러옴
                if idx == 0: 
                    self.filenames.extend(data)
    ...
    def __getitem__(self, t):
        ...
        # AUGMENTATION true when training
        if self.augment:
            # DOWNSAMPLING
            selected_idx = np.random.choice(
                coordinates.shape[0], self.downsampling, replace=False
            ) #랜덤한 index를 downsampling 개수 만큼 채택
            selected_idx = np.sort(selected_idx) #오름차순으로 정렬
            coordinates = coordinates[selected_idx] 
            features = features[selected_idx]
        ...
        #quantization_size = smallest distance btw coordinate
        discrete_coords, unique_map, inverse_map = ME.utils.sparse_quantize(
            coordinates=coordinates,
            return_index=True,
            return_inverse=True,
...
class swav_wrapper_ssl(torch.utils.data.Dataset):
    ...
    def __getitem__(self, t):
        ...
        if t < self.len_lab:
            sup_data1 = self.lab_dataset.__getitem__(t)
            sup_selected_idx = sup_data1[3]
            sup_inverse_map = sup_data1[5]
            sup_selected_idx_ = sup_selected_idx[sup_inverse_map]
            self.lab_dataset.set_fixed_selected_idx(sup_selected_idx_)  # 고정된 인덱스 설정
            sup_data2 = self.lab_dataset.__getitem__(t)
            lab_data = sup_data2[:-1] + sup_data1  # 결합 순서 변경
            self.lab_dataset.set_fixed_selected_idx(None)  # 고정된 인덱스를 해제
        else: 
            idx = int(np.random.choice(self.len_lab, 1, replace=False))
            sup_data1 = self.lab_dataset.__getitem__(idx)
            sup_selected_idx = sup_data1[3]
            sup_inverse_map = sup_data1[5]
            sup_selected_idx_ = sup_selected_idx[sup_inverse_map]
            self.lab_dataset.set_fixed_selected_idx(sup_selected_idx_)
            sup_data2 = self.lab_dataset.__getitem__(idx)
            lab_data = sup_data2[:-1] + sup_data1  # 결합 순서 변경
            self.lab_dataset.set_fixed_selected_idx(None)

        # Unsupervised data (unlabeled)
        if t < self.len_unlab:
            unsup_data1 = self.unlab_dataset.__getitem__(t)
            unsup_selected_idx = unsup_data1[3]
            unsup_inverse_map = unsup_data1[5]
            unsup_selected_idx_ = unsup_selected_idx[unsup_inverse_map]
            self.unlab_dataset.set_fixed_selected_idx(unsup_selected_idx_)
            unsup_data2 = self.unlab_dataset.__getitem__(t)
            unlab_data = unsup_data2[:-1] + unsup_data1  # 결합 순서 변경
            self.unlab_dataset.set_fixed_selected_idx(None)
        else:
            idx = int(np.random.choice(self.len_unlab, 1, replace=False))
            unsup_data1 = self.unlab_dataset.__getitem__(idx)
            unsup_selected_idx = unsup_data1[3]
            unsup_inverse_map = unsup_data1[5]
            unsup_selected_idx_ = unsup_selected_idx[unsup_inverse_map]
            self.unlab_dataset.set_fixed_selected_idx(unsup_selected_idx_)
            unsup_data2 = self.unlab_dataset.__getitem__(idx)
            unlab_data = unsup_data2[:-1] + unsup_data1  # 결합 순서 변경
            self.unlab_dataset.set_fixed_selected_idx(None)

        # Concatenate supervised and unsupervised data
